...
`harness.js` executes this module with fake local tools and returns a normalized trace. The pull request workflow uses that repository-local harness, so scenario evidence is generated from the checked-out revision without calling a model or external service.
