...
| Metric | Result |
| --- | ---: |
| Public JS/TS agent repo seeds tested | 25 |
| Repos scanned | 20/25 |
| Skipped repos | 5 |
| Crashes | 0 |
| Useful findings | 48 |
...
| --- | ---: | --- | --- |
| `vercel/ai` | 147 -> 1 | `@/tool/weather-tool` from `examples/ai-e2e-next/agent/anthropic/tools-agent.ts`; `@/tool/sandbox-shell-tool` from `examples/ai-e2e-next/agent/openai/sandbox-agent.ts` | The cited `@/tool/*` blockers now resolve; the remaining alias sample is CSS. |
| `i-am-bee/beeai-framework` | 117 -> 0 | `@/agents/base.js`, `@/memory/base.js`, and `@/backend/message.js` from `typescript/src/adapters/a2a/agents/agent.ts` | The cited runtime agent/framework aliases now resolve. |
| `VoltAgent/voltagent` | 56 -> 9 | `@/voltagent` and `@/lib/ai/config` from `examples/next-js-chatbot-starter-template/app/api/chat/route.ts` | The cited `@/` blockers now resolve; remaining samples use `#/` aliases. |

Decision: high-confidence project-local `@/` and `~/` alias reachability is worth supporting because the samples include agent files, tool files, and API routes importing local project modules. The current resolver handles those cases only when a target file exists under a nearby project root or `src` root. This should not become a universal TypeScript resolver.
...
- Remaining alias-like samples are now small enough to inspect directly instead of treating them as a broad resolver emergency.
